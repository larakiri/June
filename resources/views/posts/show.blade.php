                ...
                <div class="col-span-4 lg:text-center lg:pt-14 mb-10">
                    ...
                    <div class="py-2">
                        @if ( auth()->user()->followings->where('following_id', $post->author->id)->first() == null )
                            <form method="POST" action='/admin/{{ $post->author->username }}/followings'>
                                @csrf
    
                                <button type="submit" class="hover:underline bg-red-200 rounded-xl px-16 mr-10">Follow</button>
                            </form>
                        @else
                            <form method="POST" action="/admin/followings/{{ $post->author->username }}">
                                @csrf
                                @method('DELETE')

                                <button type="submit" class="hover:underline bg-red-200 rounded-xl px-16 mr-10">Unfollow</button>
                            </form>
                        @endif
                    </div>
                    @endauth
